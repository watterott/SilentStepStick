...
function connectButtonClicked() {
    ...
    UI_Chip_comboBox.setEnabled(true);
}
...
function showRegister(register) {
    ...
    for (var n = 0; n < register.bitName.length; n++) {
        ...
        oldText = register.bitName[n].treeItem.text(1);
        var digits = 1 + ((register.bitName[n].len - 1) >> 2); // number of hex digits to display
        newText = "   " + decimalToHex(value, digits);
            ...
            if (receivedData[1] === 0) // echo datagram
            {
                if (receivedData[2] & 0x80) // write request
                {
                    if (receivedData.length >= 8) {
                        for (i = 0; i < 8; i++) // remove echo datagram
                            receivedData.shift();
                        packetsReceived++;
                ...
                {
                    for (i = 0; i < 4; i++) // remove echo datagram
                        receivedData.shift();
                }
...
function readAllRegisters() {
    for (var i = 0; i < AllRegisters.length; i++) {
        if (AllRegisters[i].rw_info != "Write only" && (AllRegisters[i].chips && AllRegisters[i].chips.indexOf(currChip) >= 0))
...
function clearTreeWidget(treeWidget) {
    var n = treeWidget.topLevelItemCount();
    for (var i = 0; i < n; i++) {
...
function initTreeWidget(Registers, treeWidget) {
    ...
    for (var i = 0; i < Registers.length; i++) {
        ...
        if (Registers[i].chips && Registers[i].chips.indexOf(currChip) >= 0) {
	...
	//********************* GeneralRegisters ******************
    {
        ...
        bitName: [
            ...
            },
            {
                ...
                bit: 2,
                len: 1,
                checkBox: UI_EN_SPREADCYCLE_checkBox
            },
            {
                name: "SHAFT",
                bit: 3,
        ...
        rw_info: "R/W",
        chips: [chip.TMC2208, chip.TMC2209]
	},
    {
        name: "GSTAT",
        addr: 0x01,
        bitName: [
            ...
            },
            {
                ...
                bit: 1,
                len: 1,
                checkBoxColor: UI_checkBox_15
            },
            {
                name: "UV_CP",
                bit: 2,
                len: 1,
                checkBoxColor: UI_checkBox_16
            },
		],
        rw_info: "R/W Clear",
        chips: [chip.TMC2208, chip.TMC2209]
        ...
        bitName: [
            {
                ...
                bit: 0,
                len: 5,
                spinBox: UI_spinBox_I_HOLD,
                callback: UI_I_HOLD_callback
            },
            {
                name: "IRUN",
                bit: 8,
                len: 5,
                spinBox: UI_spinBox_I_RUN,
                callback: UI_I_RUN_callback
            },
            {
                name: "IHOLDDELAY",
                bit: 16,
                len: 4,
                spinBox: UI_spinBox_I_HOLD_DELAY,
                callback: UI_I_HOLD_DELAY_callback
            },
		],
        rw_info: "Write only",
        chips: [chip.TMC2208, chip.TMC2209]
        ...
        bitName: [
            {
                ...
                bit: 0,
                len: 8,
                spinBox: UI_spinBox_T_POWER_DOWN,
                callback: UI_TPOWERDOWN_callback
            },
		],
        rw_info: "Write only",
        chips: [chip.TMC2208, chip.TMC2209]
...
//			{ name: "-", bit: 11, len: 4},
            {
                ...
                bit: 7,
                len: 4,
                spinBox: UI_HEND_spinBox
            },
            {
                name: "HSTRT",
                bit: 4,
                len: 3,
                spinBox: UI_HSTRT_spinBox
            },
            {
                name: "TOFF",
                bit: 0,
                len: 4,
                spinBox: UI_TOFF_spinBox
            },
		],
        rw_info: "R/W",
        chips: [chip.TMC2208, chip.TMC2209]
        ...
        bitName: [
            {
                ...
                bit: 31,
                len: 1,
                checkBoxColor: UI_checkBox_1
            },
            {
                name: "STEALTH",
                bit: 30,
                len: 1,
                checkBoxColor: UI_checkBox_2
            },
//			{ name: "-", bit: 24, len: 6},
//			{ name: "-", bit: 21, len: 3},
            {
...
//			{ name: "-", bit: 12, len: 4},
            {
                ...
                bit: 11,
                len: 1,
                checkBoxColor: UI_checkBox_3
            },
            {
                name: "T150",
                bit: 10,
                len: 1,
                checkBoxColor: UI_checkBox_4
            },
            {
                name: "T143",
                bit: 9,
                len: 1,
                checkBoxColor: UI_checkBox_5
            },
            {
                name: "T120",
                bit: 8,
                len: 1,
                checkBoxColor: UI_checkBox_6
            },
            {
                name: "OLB",
                bit: 7,
                len: 1,
                checkBoxColor: UI_checkBox_9
            },
            {
                name: "OLA",
                bit: 6,
                len: 1,
                checkBoxColor: UI_checkBox_10
            },
            {
                name: "S2VSB",
                bit: 5,
                len: 1,
                checkBoxColor: UI_checkBox_11
            },
            {
                name: "S2VSA",
                bit: 4,
                len: 1,
                checkBoxColor: UI_checkBox_12
            },
            {
                name: "S2GB",
                bit: 3,
                len: 1,
                checkBoxColor: UI_checkBox_13
            },
            {
                name: "S2GA",
                bit: 2,
                len: 1,
                checkBoxColor: UI_checkBox_14
            },
            {
                name: "OT",
                bit: 1,
                len: 1,
                checkBoxColor: UI_checkBox_7
            },
            {
                name: "OTPW",
                bit: 0,
                len: 1,
                checkBoxColor: UI_checkBox_8
            }
		],
        rw_info: "Read only",
        chips: [chip.TMC2208, chip.TMC2209]
        ...
        bitName: [
            {
                ...
                bit: 28,
                len: 4,
                spinBox: UI_PWM_LIM_spinBox
            },
            {
                name: "PWM_REG",
                bit: 24,
                len: 4,
                spinBox: UI_PWM_REG_spinBox
            },
//			{ name: "-", bit: 23, len: 1},
//			{ name: "-", bit: 22, len: 1},
            {
            ...
            },
            {
                ...
                bit: 19,
                len: 1,
                checkBox: UI_PWM_AUTOGRAD_checkBox
            },
            {
                name: "PWM_AUTOSCALE",
                bit: 18,
                len: 1,
                checkBox: UI_PWM_AUTOSCALE_checkBox
            },
            {
                name: "PWM_FREQ",
                bit: 16,
                len: 2,
                spinBox: UI_PWM_FREQ_spinBox,
                callback: UI_PWM_FREQ_callback
            },
            {
                name: "PWM_GRAD",
                bit: 8,
                len: 8,
                spinBox: UI_PWM_GRAD_spinBox
            },
            {
                name: "PWM_OFS",
                bit: 0,
                len: 8,
                spinBox: UI_PWM_OFS_spinBox
            }
		],
        rw_info: "R/W",
        chips: [chip.TMC2208, chip.TMC2209]
...
function UI_TCOOLTHRS_spinBoxValueChanged(value) {
    setRegisterByName("TCOOLTHRS", value);
}

function UI_SGTHRS_spinBoxValueChanged(value) {
    setRegisterByName("SGTHRS", value);
}

function UI_SEIMIN_spinBoxValueChanged(value) {
    setRegisterByName("SEIMIN", value);
}

function UI_SEDN_spinBoxValueChanged(value) {
    setRegisterByName("SEDN", value);
}

function UI_SEMAX_spinBoxValueChanged(value) {
    setRegisterByName("SEMAX", value);
}

function UI_SEUP_spinBoxValueChanged(value) {
    setRegisterByName("SEUP", value);
}

function UI_SEMIN_spinBoxValueChanged(value) {
    setRegisterByName("SEMIN", value);
}
...
initTreeWidget(AllRegisters, UI_GeneralTreeWidget);
UI_Chip_comboBox.currentTextChangedSignal.connect(chipChanged);
...
// StallGuard
UI_TCOOLTHRS_spinBox.valueChangedSignal.connect(UI_TCOOLTHRS_spinBoxValueChanged);
UI_SGTHRS_spinBox.valueChangedSignal.connect(UI_SGTHRS_spinBoxValueChanged);
UI_SEIMIN_spinBox.valueChangedSignal.connect(UI_SEIMIN_spinBoxValueChanged);
UI_SEDN_spinBox.valueChangedSignal.connect(UI_SEDN_spinBoxValueChanged);
UI_SEMAX_spinBox.valueChangedSignal.connect(UI_SEMAX_spinBoxValueChanged);
UI_SEUP_spinBox.valueChangedSignal.connect(UI_SEUP_spinBoxValueChanged);
UI_SEMIN_spinBox.valueChangedSignal.connect(UI_SEMIN_spinBoxValueChanged);
